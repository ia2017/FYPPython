...
# ------ Opening config.cfg to change to DIRECT ------
config1=open('configunswept.cfg','r')
...
lines1[math]='MATH_PROBLEM= DIRECT' + '\n'
config1=open('configunswept.cfg','w')
config1.writelines(lines1)
config1.close()
...
# ---- Initiate solver ----
p1 = subprocess.run(['parallel_computation_fsi.py','-f', filename, "-n", str(nproc)])

print("-------Checkpoint-----")
# Renaming files
#os.rename('fsisteady_0.csv','fsisteady_0.csv')
#os.rename('fsisteady_0.dat','fsisteady_0.dat')
#os.rename('fsisteady_1.csv','fsisteady_1.csv')
#os.rename('fsisteady_1.dat','fsisteady_1.dat')
...
lines1[math]='MATH_PROBLEM= DISCRETE_ADJOINT' + '\n'
config1=open('configunswept.cfg','w')
config1.writelines(lines1)
config1.close()
...
for opt in convnum_all:
    ...
    p1 = subprocess.run(['mpiexec', '-np', str(nproc),'SU2_CFD_AD',filename])
